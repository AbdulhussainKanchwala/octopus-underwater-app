import { LAYOUT_DETACHED, LAYOUT_HORIZONTAL, LAYOUT_TWO_COLUMN_MENU, LAYOUT_WIDTH_FLUID, LEFT_SIDEBAR_THEME_DARK, LEFT_SIDEBAR_TYPE_DEFAULT, MENU_POSITION_FIXED, TOPBAR_THEME_LIGHT } from "../config/layout.model";
...
const findMenuItem = (menuItems: MenuItem[], menuItemKey: string): any => {
    if (menuItems && menuItemKey) {
        for (const element of menuItems) {
            ...
            var found = findMenuItem(element.children, menuItemKey);
...
const getLayoutConfig = (layoutType: string): LayoutConfig => {
    let config: LayoutConfig = {
        ...
        leftSidebarTheme: sessionStorage.getItem("Leftsidebar_Theme") || LEFT_SIDEBAR_THEME_DARK,
